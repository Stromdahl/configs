...
- [x] Every service in the compose stack runs as a non-root user, or its need for
      root is explicitly documented (e.g. a sidecar that legitimately requires it).
- [x] Containers that don't need extra Linux capabilities run with them dropped and
      `no-new-privileges` set.
- [x] Bind mounts are read-only wherever the service only reads (e.g. media for
      Jellyfin), and writable mounts are scoped to the minimum path.
- [x] All services still function after hardening: Jellyfin transcodes on the iGPU,
      gluetun's tunnel is up, the *arr stack reads/writes its volumes, Immich and
      Paperless ingest normally.
- [x] No use of host-wide `userns-remap` (the per-service approach is preserved).
